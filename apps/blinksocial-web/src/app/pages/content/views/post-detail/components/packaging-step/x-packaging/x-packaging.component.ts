...
@Component({
  ...
  templateUrl: './x-packaging.component.html',
  styleUrl: './x-packaging.component.scss',
})
export class XPackagingComponent {
  ...
  readonly value = input<PackagingXContract | undefined>(undefined);
  readonly disabled = input(false);

  readonly valueChange = output<PackagingXContract>();
  ...
  protected readonly keywords = computed(() => this.value()?.keywords ?? []);
  ...
  protected onKeywordsChange(keywords: string[]): void {
    this.patch({ keywords });
  }
  ...
  private patch(delta: Partial<PackagingXContract>): void {
