    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="general" className="w-full">
        <TabsList className="bg-white border w-full justify-start h-12 p-1 gap-1 mb-6 overflow-x-auto">
          ...
          <TabsTrigger value="content" className="gap-2 px-4 data-[state=active]:bg-[#d94e33] data-[state=active]:text-white">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <Label className="flex items-center gap-2 text-base font-bold text-gray-900">
                    ...
                    <FieldInfo content="Define and manage the specific demographic segments you are targeting." />
                  </Label>
                  <Button onClick={addSegment} variant="outline" size="sm" className="h-8 border-[#d94e33] text-[#d94e33] hover:bg-[#d94e33]/5">
                    <Plus className="mr-2 size-3" /> Add Segment
                  </Button>
                </div>
                <div className="space-y-3">
                  {audienceSegments.map((segment) => (
                    <motion.div 
                      layout 
                      key={segment.id}
                      className="group relative grid grid-cols-1 md:grid-cols-[1fr_200px] gap-4 p-4 border rounded-xl bg-gray-50/50 hover:bg-white hover:shadow-sm transition-all"
                    >
                      <div className="space-y-2">
                        <Label className="text-xs text-muted-foreground uppercase font-bold">Demographic Description</Label>
                        <Input 
                          defaultValue={segment.description} 
                          placeholder="e.g. Tech-savvy engineers interested in AI automation"
                          className={cn(fieldStyles, "bg-white border border-gray-100 shadow-none")}
                        />
                      </div>
                      <div className="space-y-2">
                        <Label className="text-xs text-muted-foreground uppercase font-bold">Age Range</Label>
          ...
          <Card className="border-none shadow-sm bg-white/80 backdrop-blur-sm">
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Manage your content pillars, audience mapping, and target platforms.
                </CardDescription>
              </div>
              <Button onClick={addPillar} variant="outline" size="sm" className="h-8 border-[#d94e33] text-[#d94e33] hover:bg-[#d94e33]/5">
                <Plus className="mr-2 size-3" /> Add Pillar
              </Button>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-6">
                {contentPillars.map((pillar) => (
                  <motion.div 
                    layout
                    key={pillar.id}
                    className="relative p-6 border rounded-2xl bg-white shadow-sm space-y-4 group transition-all hover:bg-gray-50/30"
                  >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label className="text-xs font-bold uppercase text-muted-foreground tracking-tight flex items-center gap-2">
                          Pillar Name
                          <FieldInfo content="A clear name for this specific content pillar." />
                        </Label>
                        <Input 
                          value={pillar.name} 
                      ...
                      <div className="space-y-2">
                        <Label className="text-xs font-bold uppercase text-muted-foreground tracking-tight flex items-center gap-2">
                          Themes
                          <FieldInfo content="Core thematic topics that define this content pillar." />
                        </Label>
                        <Input 
                          value={pillar.themes} 
                          onChange={(e) => updatePillar(pillar.id, "themes", e.target.value)}
                          placeholder="AI, Development, Tutorials"
                          className={cn(fieldStyles, "h-11")}
                        />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <Label className="text-xs font-bold uppercase text-muted-foreground tracking-tight flex items-center gap-2">
                        Description
                        <FieldInfo content="Explain the goal and focus of this content pillar." />
                      </Label>
                      <Textarea 
                        value={pillar.description}
                        onChange={(e) => updatePillar(pillar.id, "description", e.target.value)}
                        placeholder="Purpose of this pillar..."
                        className={cn(fieldStyles, "min-h-[80px] py-4")}
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                      <div className="space-y-2">
                        <Label className="text-[10px] font-bold uppercase text-muted-foreground flex items-center gap-2">
                          <Users className="size-3" /> Audience Mapping
                          <FieldInfo content="Map this pillar to specific audience segments." />
                        </Label>
                        <div className="flex flex-wrap gap-1.5">
                          {AVAILABLE_AUDIENCES.map(audience => {
                            const isSelected = pillar.audienceSegments.includes(audience);
                            return (
                              <button
                                key={audience}
                                onClick={() => toggleSegmentOnPillar(pillar.id, audience)}
                                className={cn(
                                  "px-2.5 py-0.5 rounded-full text-[10px] font-medium border transition-all flex items-center gap-1",
                                  isSelected 
                                    ? "bg-[#d94e33] border-[#d94e33] text-white shadow-sm" 
                                    : "bg-gray-50 border-gray-200 text-gray-500 hover:border-[#d94e33]/50"
                                )}
                              >
                      ...
                      <div className="space-y-2">
                        <Label className="text-[10px] font-bold uppercase text-muted-foreground flex items-center gap-2">
                          <Share2 className="size-3" /> Target Platforms
                          <FieldInfo content="Choose platforms where this pillar's content will be posted." />
                        </Label>
                        <div className="flex flex-wrap gap-1.5">
                          {AVAILABLE_PLATFORMS.map(platform => {
                            ...
                            return (
                              <button
                                key={platform}
                                onClick={() => togglePlatformOnPillar(pillar.id, platform)}
                                className={cn(
                                  "px-2.5 py-0.5 rounded-full text-[10px] font-medium border transition-all flex items-center gap-1",
                                  isSelected 
                                    ? "bg-blue-600 border-blue-600 text-white shadow-sm" 
                                    : "bg-gray-50 border-gray-200 text-gray-500 hover:border-blue-600/50"
                                )}
                              >
                                {isSelected && <Check className="size-2.5" />}
                                {platform}
            ...
            <CardFooter className="border-t bg-gray-50/50 p-6 flex justify-end">
              <Button className="bg-[#d94e33] hover:bg-[#c2462e]">
                <Save className="mr-2 size-4" /> Save Content Strategy
              </Button>
            </CardFooter>
          </Card>
        </TabsContent>
